...
d <- read_dta("/Users/iandl/Downloads/FF_wave6_2019.dta")
forplot <- d %>%
  filter(!is.na(k6natwt)) %>%
  arrange(-k6natwt) %>%
  mutate(prop_weight = cumsum(k6natwt) / sum(k6natwt),
         prop_sample = (1:n()) / n()) %>%
...
forplot %>%
  ...
  theme(axis.text = element_text(face = "bold"),
        axis.title = element_blank()) +
  ggsave("/Users/iandl/Downloads/weight_issue_3_y15.pdf",
         height = 1.8, width = 1.9)
