...
\node[align = left, anchor = south west, font = \scriptsize, text width = \textwidth] at (-1, -.33) {\textbf{NOTE:} These slides are for a pedagogical workshop.\\The main slides for this paper are \bref{https://github.com/ilundberg/slides/blob/master/what_is_your_estimand/what_is_your_estimand_slides.pdf}{here}.};
\node[align = left, anchor = south west, font = \scriptsize, text width = \textwidth] at (-1, -.65) {6 October 2021. University of Wisconsin CDE Training Workshop. \\Paper in \emph{American Sociological Review}. Preprint on \bref{https://doi.org/10.31235/osf.io/ba67n}{SocArxiv}. Replication code on  \bref{https://doi.org/10.7910/DVN/ASGOVU}{Dataverse}. Research reported in this publication was supported by The Eunice Kennedy Shriver National Institute of Child Health \& Human Development of the National Institutes of Health under Award Number P2CHD047879.};
\end{tikzpicture}
\end{frame}
...
{\begin{tikzpicture}[x = 1.1in, y = .3in]
 ...
 \draw<4>[->, red, line width = 1.5pt] (1.7,-2) -- (1.9,-1.2);
    ...
    \onslide<20->{
    \node[anchor = west, font = \large] at (-.5,-5.5) {\bgray{Plan for today}};
    \node[anchor = west, font = \large]  at (-.5,-6.5) {--- Intro: What this looks like in practice};
    \node[anchor = west, font = \large]  at (-.5,-7.5) {--- Example: Revisiting a published paper};
    \node[anchor = west, font = \large]  at (-.5,-8.5) {--- Group exercises: Together we will state precise research goals};
    }
    \end{tikzpicture}
    }
\end{frame}
...
\begin{frame}[t]\headerfigurelink \vskip 1in
\huge 2. Link to observables
\end{frame}

% ANIMATE THE FUNDAMENTAL PROBLEM OF CAUSAL INFERENCE
\begin{frame}[t]\headerfigurelink
\begin{tikzpicture}[x = \textwidth, y = .8\textheight]
\node at (0,0) {};
\node at (1,1) {};
\node<1-2>[anchor = west, font = {\large\bf}] (goal) at (0, .9) {Causal effect in a population};
\draw<1-2>[line width = 1.5pt, line cap = round] (goal.south west) -- (goal.south east);
\node<1-2>[anchor = west] at (0, .75) {What is the causal effect of motherhood on employment};
\node<1-2>[anchor = west] at (0, .68) {among U.S. resident women ages 21--35?};
% Title rows
\node[font = \footnotesize, anchor = east] at (.2, .3) {Woman 1};
\node[font = \footnotesize, anchor = east] at (.2, .25) {Woman 2};
\node[font = \footnotesize, anchor = east] at (.2, .2) {Woman 3};
...
\draw[thick] (.62,.35) -- (.78, .35);
% Cell values
% 1 and 2 are mothers, 3 and 4 are non-mothers
\node[font = \footnotesize] at (.3, .3) {0};
\node[font = \footnotesize] at (.3, .25) {0};
\node<1>[font = \footnotesize] at (.3, .2) {0};
\node<1>[font = \footnotesize] at (.3, .15) {1};
\node<2-4>[font = \footnotesize, gray] at (.3, .2) {?};
\node<2-4>[font = \footnotesize, gray] at (.3, .15) {?};
\node<1>[font = \footnotesize] at (.5, .3) {1};
\node<1>[font = \footnotesize] at (.5, .25) {0};
\node<2-5>[font = \footnotesize, gray] at (.5, .3) {?};
\node<2-5>[font = \footnotesize, gray] at (.5, .25) {?};
\node[font = \footnotesize] at (.5, .2) {1};
\node[font = \footnotesize] at (.5, .15) {1};
\node<1>[font = \footnotesize] at (.7, .3) {-1};
\node<1>[font = \footnotesize] at (.7, .25) {0};
\node<1>[font = \footnotesize] at (.7, .2) {-1};
\node<1>[font = \footnotesize] at (.7, .15) {0};
\node<2-6>[font = \footnotesize, gray] at (.7, .3) {?};
\node<2-6>[font = \footnotesize, gray] at (.7, .25) {?};
\node<2-6>[font = \footnotesize, gray] at (.7, .2) {?};
\node<2-6>[font = \footnotesize, gray] at (.7, .15) {?};
% Box for among subgroup
\only<4->{
\draw[line width = 2pt, rounded corners, gray] (.05,.05) rectangle (.85,.6);
\node[anchor = south west, align = left, font = \footnotesize] at (.05,.65) {\bgray{Subpopulation}\\--- Age 30\\--- College-educated\\--- Employed last year};
}
% Fill in the Y(1) values
\draw<5->[->, gray, thick] (.35,.275) to[bend left = 45] (.35, .175); 
\node<5->[font = \footnotesize, gray] at (.3, .2) {0?};
\node<5->[font = \footnotesize, gray] at (.3, .15) {0?};
% Fill in the Y(0) values
\draw<6->[->, gray, thick] (.55,.175) to[bend right = 45] (.55, .275); 
\node<6->[font = \footnotesize, gray] at (.5, .3) {1?};
\node<6->[font = \footnotesize, gray] at (.5, .25) {1?};
% Fill in the causal effects values
\node<7->[font = \footnotesize, gray] at (.7, .3) {-1?};
\node<7->[font = \footnotesize, gray] at (.7, .25) {-1?};
\node<7->[font = \footnotesize, gray] at (.7, .2) {-1?};
\node<7->[font = \footnotesize, gray] at (.7, .15) {-1?};
% DAG
\onslide<8-10> {
\node[font = \footnotesize] (age) at (.56,.96) {Age};
\node[font = \footnotesize] (college) at (.56,.83) {College};
\node[font = \footnotesize, align = center] (lag) at (.56,.7) {Lagged\\employment};
\draw[->, thick] (age) -- (college);
\draw[->, thick] (college) -- (lag);
\node[font = \footnotesize] (motherhood) at (.73,.83) {Motherhood};
\node[font = \footnotesize] (employment) at (.9,.83) {Employment};
\draw[->, thick] (college) -- (motherhood);
\draw[->, thick] (college) to[bend right] (employment);
\draw[->, thick] (age) -- (motherhood);
\draw[->, thick] (age) to[out = 0, in = 120] (employment);
\draw[->, thick] (.63,.75) -- (motherhood);
\draw[->, thick] (lag) to[out = 0, in = 240] (employment);
}
\onslide<9>{
\node[font = {\footnotesize\bf}, red] (u) at (.73,.93) {$U$};
\draw[->, line width = 1.5pt, red] (u) -- (motherhood);
\draw[->, line width = 1.5pt, red] (u) -- (employment);
}
\onslide<11>{
\node[anchor = south west, align = left, font = \footnotesize] at (.55,.65) {\bgray{Getting the population}\\1) Do this in every subpopulation\\2) Aggregate over subpopulations\\};
}
\end{tikzpicture}
\end{frame}
...
\begin{frame}[t]\headerfigurelearn \vskip 1in
\huge 3. Learn from data
\end{frame}

\begin{frame}[t]\headerfigurelearn
\begin{tikzpicture}[x = \textwidth, y = .8\textheight]
\node at (0,0) {};
\node at (1,1) {};
\node[anchor = south west, align = left, font = \footnotesize] at (.05,.65) {\bgray{Subpopulation}\\--- Age 30\\--- College-educated\\--- Employed last year};
...
\node[font = \footnotesize] at (.3, .25) {0};
% Fill in the Y(1) values
\draw[->, gray, thick] (.35,.275) to[bend left = 45] (.35, .175); 
\node[font = \footnotesize, gray] at (.3, .2) {0?};
\node[font = \footnotesize, gray] at (.3, .15) {0?};
% LEARN
\onslide<2->{
\node[anchor = south west, align = center, font = \large] at (.7,.15) {\bblue{Learn}\\Do this better\\with machine\\learning};
\draw[->, line width = 2pt, blue, dashed] (.65,.25) -- (.5,.25);
}
\end{tikzpicture}
\end{frame}
...
\begin{frame}[t]\headerfigure  \vskip 1in
\huge Example
\end{frame}
...
\node at (0,1.03) {};
\node at (1,0) {};
\node<1-5>[anchor = south east, gray] at (1,.02) {Angrist and Evans 1998};
%\node[anchor = north west, font = \small, align = left] at (0, .9) {Effect of motherhood\\on employment};
\node<1-2>[font = \footnotesize] (t) at (.57,.8) {Motherhood};
\node<3-7>[font = \footnotesize] (t) at (.57,.8) {Third birth};
\node<1-7>[font = \footnotesize] (y) at (.8,.8) {Employment};
\draw<1-7>[->, line width = 2pt, blue] (t) -- (y);
\node<4-7>[font = \footnotesize, align = center] (z) at (.34,.8) {First two\\births are\\the same sex};
\draw<4-7>[->, line width = 2pt, purple] (z) -- (t);
\node<2-7>[font = \footnotesize, anchor = north] (u) at (.7,.95) {$U$};
\draw<2-7>[->, thick] (u) -- (y);
\draw<2-7>[->, thick] (u) -- (t);
% Introduce that the instrument -> outcome effect is identified
\node<5-7>[font = \footnotesize, align = center] (zTotal) at (.34,.6) {First two\\births are\\the same sex};
\node<5-7>[font = \footnotesize] (yTotal) at (.8,.6) {Employment};
\draw<5-7>[->, line width = 2pt, seagreen4] (zTotal) -- (yTotal);
% Walk through mechanics of how this works
\only<6-7>{
\node[anchor = north west] (mechanics) at (.33,.4) {Mechanics of instrumental variables};
\draw[line width = 1pt, line cap = rounded] (mechanics.south west) -- (mechanics.south east);
\draw[->, line width = 2pt, blue] (.45,.2) -- (.52,.2);
\node at (.6, .2) {$=$};
\draw[->, line width = 2pt, seagreen4] (.7,.25) -- (.77,.25);
\draw[->, line width = 2pt, purple] (.7,.15) -- (.77,.15);
\draw[thick] (.66,.2) -- (.81,.2);
\node[font = \footnotesize] at (.6,.07) {(Note: This can be made more precise)};
}
% But wait: not everyone affected
\only<7>{
\node[red, font = \bf, anchor = south, font = {\small\bf}] (wait) at (.12,.85) {But wait!};
\draw[line width = 1.5pt, red, line cap = rounded] (wait.south west) -- (wait.south east);
\node[anchor = north, align = center, font = \small] at (.12,.85) {Not everyone is\\affected by this\\instrument};
}
% Begin subpopulations
\only<8-14>{
\node[anchor = north west, font = {\small\bf}, gray] (header) at (0,1) {Four Subpopulations};
\node[anchor = north east, font = \small, gray] at (1,1) {(Angrist, Imbens, \& Rubin 1996)};
}
%\draw<6->[line width = 1.5pt, gray, line cap = rounded] (header.south west) -- (header.south east);
% Population 1: Always takers
\only<9-14>{
\node[anchor = north west, align = left, font = \footnotesize] at (0, .93) {\bgray{Always-Takers.}};
\node[anchor = north west, align = left, font = \footnotesize] at (.23, .93) {Third kid no matter what.};
\node[font = \footnotesize, align = center, anchor = west] (z1) at (0,.8) {First two births same sex};
\node[font = \footnotesize] (t1) at (.5,.8) {Third birth};
\node[font = \footnotesize] (e1) at (.8,.8) {Employment};
\draw[->, thick] (t1) -- (e1);
\draw[->, thick, dashed] (z1) to node[midway, above, font = \footnotesize] {0} (t1);
\draw[<->, thick, dashed, red] (t1) to[bend left = 15] (e1);
}
% Population 2: Never takers
\only<10-14>{
\node[anchor = north west, align = left, font = \footnotesize] at (0, .7) {\bgray{Never-Takers.}};
\node[anchor = north west, align = left, font = \footnotesize] at (.23, .7) {Sticking with two kids, no matter what.};
\node[font = \footnotesize, align = center, anchor = west] (z2) at (0,.57) {First two births same sex};
\node[font = \footnotesize] (t2) at (.5,.57) {Third birth};
\node[font = \footnotesize] (e2) at (.8,.57) {Employment};
\draw[->, thick] (t2) -- (e2);
\draw[->, thick, dashed] (z2) to node[midway, above, font = \footnotesize] {0} (t2);
\draw[<->, thick, dashed, red] (t2) to[bend left = 15] (e2);
}
% Population 3: Compliers
\only<11->{
\node[anchor = north west, align = left, font = \footnotesize] at (0, .47) {\bgray{Compliers.}};
\node[anchor = north west, align = left, font = \footnotesize] at (.23, .47) {Third kid if and only if the first two are the same sex.};
\node[font = \footnotesize, align = center, anchor = west] (z3) at (0,.34) {First two births same sex};
\node[font = \footnotesize] (t3) at (.5,.34) {Third birth};
\node[font = \footnotesize] (e3) at (.8,.34) {Employment};
\draw[->, thick] (t3) -- (e3);
\draw[->, thick, dashed] (z3) to node[midway, above, font = \footnotesize] {+1} (t3);
\draw[<->, thick, dashed, red] (t3) to[bend left = 15] (e3);
}
% Population 4: Defiers
\only<12-14>{
\node[anchor = north west, align = left, font = \footnotesize] (defiers) at (0, .24) {\bgray{Defiers.}};
\node[anchor = north west, align = left, font = \footnotesize] at (.23, .24) {Third kid if and only if the first two are NOT the same sex.};
\node[font = \footnotesize, align = center, anchor = west] (z4) at (0,.11) {First two births same sex};
\node[font = \footnotesize] (t4) at (.5,.11) {Third birth};
\node[font = \footnotesize] (e4) at (.8,.11) {Employment};
\draw[->, thick] (t4) -- (e4);
\draw[->, thick, dashed] (z4) to node[midway, above, font = \footnotesize] {-1} (t4);
\draw[<->, thick, dashed, red] (t4) to[bend left = 15] (e4);
}
% Assume away the defiers
\draw<13-14>[thick] (defiers.west) -- (defiers.east);
\node<13-14> [anchor = west, font = \tiny, align = center] at (defiers.east) {(assumed\\away)};
\draw<14->[fill = blue, fill opacity = .2, rounded corners, draw = white] (0,.28) rectangle (.95, .48);
\node<15>[font = {\large\bf}, blue] (takeaway) at (.5,.9) {Takeaway};
\draw<15>[line width = 1.5pt, line cap = rounded, blue] (takeaway.south west) -- (takeaway.south east);
\node<15>[font = \large, align = center] at (.5,.7) {Instrumental variables estimates the\\average causal effect among\\a particular \bblue{target population}: compliers};
\end{tikzpicture}
\end{frame}
...
\node<1-4>[anchor = north west, font = \small, align = left] at (0, .9) {Effect of motherhood\\on employment};
\node<1-4>[font = \footnotesize, align = center] (z) at (.25,.32) {First two births\\are the same sex};
\node<1-4>[font = \footnotesize] (t) at (.5,.32) {Third birth};
\draw<1-4>[->, thick] (z) -- (t);
\node<1-4>[font = \footnotesize] (y) at (.7,.32) {Employed};
\node<1-4>[font = \footnotesize, anchor = north] (u) at (.6,.45) {$U$};
\draw<1-4>[->, thick] (t) -- (y);
\draw<1-4>[->, thick] (u) -- (y);
\draw<1-4>[->, thick] (u) -- (t);
\node<1-4>[anchor = north west, gray, font = \bf] at (0,.97) {Vague estimand};
\node<2->[anchor = north west, gray, font = \bf] at (.37,.97) {Precise estimand\only<7->{ B}};
\node<3->[anchor = north west, text width = .6\textwidth, font = \small, align = left] (precise1) at (.37,.9) {Effect of having \only<3>{\bblue{3 vs.~2 children}}\only<4->{3 vs.~2 children}};
\node<3>[anchor = north east, font = {\bf\footnotesize}, blue, align = center] (contrast) at (1,1.03) {unit-specific\\quantity};
\draw<3>[->, line width = 2pt, blue] (contrast) to[out = 180, in = 90] (.77,.89);
\node<4->[anchor = north west, text width = .6\textwidth, font = \small, align = left] at (.37, .84) {among those with at least two children who would have a third birth if and only if the first two were of the same sex};
\node<4>[anchor = south, font = {\bf\footnotesize}, seagreen] (population) at (.15,.6) {target population};
\draw<4>[->, line width = 2pt, seagreen] (population) to[out = 90, in = 180] (.35,.75);
\draw<4>[line width = 2pt, seagreen] (.38, .83) -- (.36, .83) -- (.36, .67) -- (.38,.67);
\draw<4>[line width = 2pt, seagreen] (.97, .83) -- (.99, .83) -- (.99, .67) -- (.97,.67);
\node<5->[font = \Large] at (.675,.6) {$\approx$ 4\% of all mothers};
% NEW THINGS BELOW
\node<6->[anchor = north west, gray, font = \bf] at (0,.97) {Precise estimand\only<7->{ A}};
\node<6->[anchor = north west, text width = .6\textwidth, font = \small, align = left] (precise1) at (0,.9) {Effect of having\\1 vs.~0 children};
\node<6->[anchor = north west, text width = .6\textwidth, font = \small, align = left] at (0, .78) {among all women};
% You have to argue
\node<8->[anchor = north west] at (0,.5) {\bgray{The estimand you choose affects your argument.}};
\node<9->[anchor = north west, font = \small] at (0,.4) {Choice A) By what assumptions can we learn that?};
\node<10->[anchor = north east, font = \small] at (1,.4) {(\bgray{link} step is hard)};
\node<11->[anchor = north west, font = \small] at (0,.3) {Choice B) Why does this quantity matter for theory?};
\node<12->[anchor = north east, font = \small] at (1,.3) {(\bgray{set} step is hard)};
\end{tikzpicture}
\end{frame}
...
\begin{frame}
\huge Group exercises
\end{frame}

\begin{frame}{Group exercises: Plan}
\begin{enumerate}
\item We will give you a vague research question
\item In groups, you will choose some way to make it precise
\item Our answers may differ. That is ok
\item We will come together and discuss
\end{enumerate}
\end{frame}
...
\node at (0,0) {};
\node at (1,0) {};
\draw[fill = gray, fill opacity = .2, rounded corners, draw = white] (.2,.5) rectangle (.8,-1.1);
\node[align = left] at (.5,0) {``Marriage has a large effect on the\\economic well-being of poor women.''};
\node[anchor = east] at (.78, -.8) {--- A policymaker};
\end{tikzpicture}
\vskip .1in
What is the estimand or target quantity? \vskip .1in
\begin{enumerate}
\item What is the unit-specific quantity?
\begin{footnotesize}
\begin{itemize}
\item[---] Define the unit of analysis
\item[---] Define the outcome
\item[---] Define the treatment variable (if any)
\item[---] Define the values to which the treatment is set (if any)
\end{itemize}
\end{footnotesize}
\item What is/are the target population(s) of units?
\end{enumerate} \vskip .1in
\textbf{Note:} There is no single right answer.
\end{frame}
...
\node at (0,0) {};
\node at (1,0) {};
\draw[fill = gray, fill opacity = .2, rounded corners, draw = white] (.15,.5) rectangle (.85,-1.1);
\node[align = left] at (.5,0) {``Those whose parents attended college are\\more likely to complete college themselves''};
\node[anchor = east] at (.83, -.8) {--- A sociologist};
\end{tikzpicture}
\vskip .1in
What is the estimand or target quantity? \vskip .1in
\begin{enumerate}
...
\node[anchor = north, align = left, font = \large] at (.5,.8) {\bblue{What is your estimand?}};
\draw[->, line width = 2pt, blue] (.615,.73) to[out = 270, in = 180] (.7,.65);
\node[anchor = north east, align = right, font = \small] at (1,.7) {The \bgray{purpose} of the\\statistical analysis};
\node[anchor = north east, align = right, font = \small] at (1,.6) {stated \bgray{outside}\\of the model};
% Begin the proposed estimand statement
\node<2->[circle, fill = lightgray, draw = lightgray, font = \footnotesize, inner sep = 8pt] (point1) at (.18,.6) {};
\node<13>[font = \scriptsize] at (point1) {$Y_i$};
\node<14->[font = \scriptsize] at (point1) {$Y_i(t)$};
\node<2->[gray, font = \footnotesize, align = center, anchor = north] (usqNote) at (point1.south) {A \bgray{unit-specific}\\\bgray{quantity}};
\node<5->[circle, fill = lightgray, draw = lightgray, font = \footnotesize, inner sep = 8pt] (point2) at (.33,.55) {};
\node<5->[circle, fill = lightgray, draw = lightgray, font = \footnotesize, inner sep = 8pt] (point3) at (.25,.4) {};
\node<5->[circle, fill = lightgray, draw = lightgray, font = \footnotesize, inner sep = 8pt] (point4) at (.4,.43) {};
\node<5->[circle, fill = lightgray, draw = lightgray, font = \footnotesize, inner sep = 8pt] (point5) at (.12,.42) {};
\node<5->[circle, fill = lightgray, draw = lightgray, font = \footnotesize, inner sep = 8pt] (point6) at (.45,.6) {};
\draw<5->[line width = 2pt, gray, rounded corners] (.05,.35) rectangle (.5,.65);
\node<5->[gray, font = \footnotesize, align = left, anchor = south west] at (.5,.35) {Aggregated over a\\\bgray{target population}};
\node<5->[font = \scriptsize] at (point1) {$Y_i(t)$};
\node<5-10>[font = \scriptsize] at (point2) {$Y_i(t)$};
\node<5->[font = \scriptsize] at (point3) {$Y_i(t)$};
\node<5-10>[font = \scriptsize] at (point4) {$Y_i(t)$};
\node<5-10>[font = \scriptsize] at (point5) {$Y_i(t)$};
\node<5->[font = \scriptsize] at (point6) {$Y_i(t)$};
\node<6->[anchor = north west, align = left] (caption1) at (0,.27) {Our framework expands \bblue{theory},};
\node<6->[anchor = north west, align = left] (caption2) at (caption1.south west) {links to transparent \bblue{evidence},};
\node<6->[anchor = north west, align = left] at (caption2.south west) {and unlocks computational \bblue{tools}};
\draw<7>[line width = 2pt, blue] (.375,.21) -- (.48,.21);
...
\draw<8-9>[line width = 2pt, blue] (.31,.145) -- (.46,.145);
\draw<10->[line width = 2pt, blue] (.425,.08) -- (.503,.08);
\node<9->[circle, draw = black, font = \footnotesize, inner sep = 8pt, line width = 2pt] at (point1) {};
\node<9->[circle, draw = black, font = \footnotesize, inner sep = 8pt, line width = 2pt] at (point3) {};
\node<9->[circle, draw = black, font = \footnotesize, inner sep = 8pt, line width = 2pt] at (point6) {};
\node<11>[font = \scriptsize] at (point2) {$\hat{Y}_i(t)$};
\node<11>[font = \scriptsize] at (point4) {$\hat{Y}_i(t)$};
\node<11>[font = \scriptsize] at (point5) {$\hat{Y}_i(t)$};
\end{tikzpicture}
\end{frame}
...
Example 1: An influential study with a \bgray{narrow theoretical estimand}
\end{frame}
...
\node<11->[anchor = north west] at (0,.5) {\bgray{You have to argue for one of two things:}};
...
\begin{frame}[t]
\headerfigure \vskip 1in
Example 2: An influential study with a misleading \bgray{link to evidence}
\end{frame}
...
\begin{frame}[t]
\headerfigure \vskip 1in
Example 3: An influential study where \bgray{estimation} led to confusion
\end{frame}
...
\node<3>[anchor = north west] (A_plot) at (0,-.5) {\includegraphics{output/gss_fourPanel_set_stage}};
\node<4->[anchor = north west] (A_plot) at (0,-.5) {\includegraphics{output/gss_fourPanel_slideFigure}};
...
\node<3-6>[anchor = north west, font = \small] at (0,.65) {--- Motivate the question};
\node<4-6>[anchor = north west, font = \small] at (0,.58) {--- Address selection};
\node<5-6>[anchor = north west, font = \small] at (0,.51) {--- Unlock computational tools};
\node<6>[anchor = north west, font = \small] at (0,.44) {--- Speak to a broad audience};
%}
%\onslide<>{
