...
\node[anchor = south, align = center, font = \footnotesize] at (.5,.35) {UCLA Sociology and\\California Center for Population Research\\ianlundberg@ucla.edu};
\node[anchor = south, align = center, font = \footnotesize] at (.5,.25) {10 Dec 2024\\Social Science in the Age of AI Symposium};
...
\item what is the scope of the intervention? \hfill \only<2->{\textcolor{blue}{\footnotesize (contribution)}}
\end{itemize}
...
\item by weighting
\item by doubly robust estimation \hfill \only<2->{\textcolor{blue}{\footnotesize (contribution)}}
\end{itemize}
\item Produce software \hfill \only<2->{\textcolor{blue}{\footnotesize (contribution)}}
\item Impact: Others using these ideas
\end{itemize}

\end{frame}

\begin{frame}{Past work: Causal decomposition analysis}
...
\node at (0,0) {};
\node at (1,1) {};
\node<2-5> at (.5,.8) {\includegraphics[width = .8\textwidth]{figures/LegewieCricco}};
\node<3-5>[anchor = north west] at (0,.5) {There are \bblue{gaps} in high school graduation by race};
\node<4-5>[anchor = north west] at (0,.4) {We could \bblue{intervene} to equalize neighborhood policing};
\node<5-5>[anchor = north west] at (0,.3) {The Black-white gap would close by more than one-fourth};
\node<6-9> at (.5,.8) {\includegraphics[width = .8\textwidth]{figures/ZhouPan}};
\node<7-9>[anchor = north west] at (0,.5) {There are \bblue{gaps} in earnings by race};
\node<8-9>[anchor = north west] at (0,.4) {We could \bblue{intervene} to promote college completion};
\node<9-9>[anchor = north west, align = left] at (0,.3) {The gap would close by more than half for men,\\but not for women};
\node<10-13> at (.5,.8) {\includegraphics[width = .8\textwidth]{figures/SchachnerWodtke}};
\node<11-13>[anchor = north west] at (0,.5) {There are \bblue{gaps} in vocabulary skills by race and class};
\node<12-13>[anchor = north west] at (0,.4) {We could \bblue{intervene} to equalize exposure to lead};
\node<13-13>[anchor = north west] at (0,.3) {Gaps would close by 15--25\%};
\end{tikzpicture}
...
\begin{frame}{Discussion: Potential outcomes and methodology} \pause
We often focus on average treatment effects:
$$\E(Y^1 - Y^0)$$ \pause
Gap-closing is a different aggregation of potential outcomes:
$$\E(Y^1 \mid X = 1) - \E(Y^1 \mid X = 0)$$ \pause
Many advances may be possible with other aggregations \vskip .1in \pause
\begin{tabular}{ll}
Effect on median & $\text{Median}(Y^1) - \text{Median}(Y^0)$ \\ \pause
Effect on variance & $\text{Var}(Y^1) - \text{Var}(Y^0)$ \\ \pause
Gap-closing for variance & $\text{Var}(Y^1\mid X = 1) - \text{Var}(Y^1 \mid X = 0)$ \\ \pause
Counterfactual slopes & $\frac{\text{Cov}(Y^1,X)}{\text{Var}(X)}$ for continuous $X$
\href{https://arxiv.org/abs/2401.07000}{\tiny (Yu \& Zhao 2024)}
\end{tabular}
\end{frame}

\begin{frame}
\begin{huge}Thanks!\end{huge} \vskip .2in
Ian Lundberg\\
\bref{https://www.ianlundberg.org}{ianlundberg.org}\\
\bref{mailto:ianlundberg@ucla.edu}{ianlundberg@ucla.edu} \vskip .2in
More about this project with a code tutorial:\\
\bref{https://ilundberg.github.io/gapclosing/github_doc/_book/index.html}{ilundberg.github.io/gapclosing}
\end{frame}
